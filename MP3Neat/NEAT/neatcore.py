...
################################
#   DO NOT UPGRADE TO NEAT-PYTHON >= 0.9.2 YET
#       IT CHANGES THE GENOME OBJECT SO THE SAVED GENOMES WILL NO LONGER BE DEPICKLED
#   NEED TO CONVERT REGISTER TO A NON-BINARY FORMAT or to decompose object into single interesting features. Might be a pain tho
################################
...
class StandardNEAT:
    def __init__(self, config_file='./config', initialState=None, recurrent=False, gpu=False, checkPoints=False):
        ...
        self.dataManager = datatools.DataManager()

    def feed(self, datapath, fetchFromHistory=False):
        self.ins, self.outs, self.labels, self.historybest = self.dataManager.preparedata(datapath, order=self.config.genome_config.num_outputs,
                                                                                   andGetHistoryBest=fetchFromHistory)

        if len(self.ins) * len(self.outs) == 0:
            raise LookupError("No inputs or no outputs fed to the core.")
    ...
    def run(self, generations=300, cores=5, showDiagrams=False):
        ...
        if self.historybest is not None:
...
class MTSNEAT(StandardNEAT):
    def feed(self, datapath, mutatingdatapath, fetchFromHistory=False):
        super().feed(datapath, fetchFromHistory=fetchFromHistory)
        self.mutatingins, self.mutatingouts, _, _ = self.dataManager.preparedata(mutatingdatapath, order=self.config.genome_config.num_outputs,
                                                                          andGetHistoryBest=fetchFromHistory)
    ...
    def run(self, generations=300, cores=5, showDiagrams=False, dataMutationRate=0.10, dataMutationInterval=100):
        ...
        self.historybest = None # otherwise they would keep being reinserted in the system
